...
const FEEDS = [
  // Kapp skips the theme gate (Travis studied under him — everything he
  // posts belongs in the stream) but gets NO special display treatment:
  // no badge, no floating. Flat feed, per Travis.
...
function parseFeed(xml, feed) {
  ...
  for (const b of blocks.slice(0, 15)) {
    ...
    out.push({ title, url: link, source: feed.name, published, themes });
  }
  return out;
}
