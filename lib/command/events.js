...
export default async function handler(req, res) {
  ...
  try {
    ...
    let rows;
    // end_date is a v10 column. The fallback keeps the feed alive on a
    // pre-Setup database — this endpoint degrading to empty would silently
    // pin both hero pages on their stale hardcoded fallbacks.
    try {
      rows = await sql`
        SELECT to_char(date, 'YYYY-MM-DD') AS date,
               to_char(end_date, 'YYYY-MM-DD') AS end_date,
               category, title, detail, full_copy, link, pin
    ...
    const events = rows.map(r => {
      ...
      // Multi-day: `end` plus its own denormalised render strings, absent
      // entirely for single-day events so old fallback data stays shaped
      // identically to live data.
      if (r.end_date) {
        const [, em, ed] = r.end_date.split('-');
        e.end = r.end_date;
        e.end_month = MONTHS[Number(em) - 1] || '';
        e.end_day = String(Number(ed));
      }
    ...
    // Short shared cache — ONLY on success. A cached degraded response would
    // pin an empty feed on the edge for minutes after a one-request DB blip
    // (it did exactly that to the first post-migrate reads).
