    ...
    if (op === 'save') {
      ...
      if (id) {
        const rows = await sql`UPDATE team_members SET name = ${name}, trigram = ${trigram},
          ...
          is_leader = ${is_leader}, manager_id = ${manager_id} WHERE id = ${id} RETURNING id`;
        ...
        const rows = await sql`INSERT INTO team_members (name, trigram, title, team_id, email, is_leader, manager_id, created_by)
          VALUES (${name}, ${trigram}, ${title}, ${team_id}, ${email}, ${is_leader}, ${manager_id}, ${who.label}) RETURNING id`;
        mid = rows[0].id;
      }
