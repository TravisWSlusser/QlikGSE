import { sql, requireScope, cors, parseBody, makeSalt, hashCode, GRANTABLE } from './auth.js';
...
export default async function handler(req, res) {
  ...
  if (['save', 'retire', 'resetCode', 'invite', 'setScopes'].includes(op) && !who.master && !who.manager) {
    return res.status(403).json({ error: 'Only managers sign the team up, invite, reset codes, grant areas, or edit members' });
  }
  // Logins stay with CORE LEADERSHIP (the manager tier) + masters — the
  // people-leader-only version accidentally locked out Travis, Barb and
  // Eric, who are leadership without direct reports. Regular staff never
  // had these controls, so "folks don't mess with logins" still holds.
  ...
  try {
    ...
    /* v12 — grant a non-manager staff member specific areas.
       Managers already hold everything, so setting scopes on one is a no-op
       waiting to confuse someone; it is refused rather than silently ignored.
       GRANTABLE is the ceiling: 'system' (maintenance switch, key minting)
       and 'projects' are not grantable this way, and auth.js re-filters on
       read too, so even a hand-edited row cannot smuggle one through. */
    if (op === 'setScopes') {
      const id = Number(b.id);
      const wanted = Array.isArray(b.scopes) ? b.scopes.map(String) : [];
      const bad = wanted.filter(s => !GRANTABLE.includes(s));
      if (bad.length) {
        return res.status(400).json({
          error: `Not grantable to a team member: ${bad.join(', ')}`,
          grantable: GRANTABLE,
          detail: "Maintenance and key minting stay with managers — use the Manager checkbox if that is really what you mean.",
        });
      }
      const scopes = GRANTABLE.filter(s => wanted.includes(s));   // dedup + fixed order
      const m = await sql`SELECT name, is_manager FROM team_members WHERE id = ${id} AND active = true LIMIT 1`;
      if (!m.length) return res.status(404).json({ error: 'No such member', id });
      if (m[0].is_manager) {
        return res.status(409).json({ error: `${m[0].name} is a manager and already holds every area` });
      }
      await sql`UPDATE team_members SET scopes = ${scopes} WHERE id = ${id}`;
      await logChange(who, 'projects', scopes.length
        ? `Gave ${m[0].name} access to ${scopes.join(', ')}`
        : `Removed ${m[0].name}'s area access`);
      return res.status(200).json({ ok: true, id, scopes });
    }
