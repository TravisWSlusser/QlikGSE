...
export default async function handler(req, res) {
  ...
  try {
    if (op === 'save') {
      ...
      if (id) {
        const rows = await sql`UPDATE team_members SET name = ${name}, trigram = ${trigram},
          title = ${title}, team_id = ${team_id} WHERE id = ${id} RETURNING id`;
        if (!rows.length) return res.status(404).json({ error: 'No such member', id });
      } else {
        const rows = await sql`INSERT INTO team_members (name, trigram, title, team_id, created_by)
          VALUES (${name}, ${trigram}, ${title}, ${team_id}, ${who.label}) RETURNING id`;
        mid = rows[0].id;
      }
    ...
    res.status(400).json({ error: 'Bad op', ops: ['save', 'retire', 'tag', 'untag'] });
  } catch (err) {
