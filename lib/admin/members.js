...
export default async function handler(req, res) {
  ...
  // 'ooo' is the one op a plain member session may run — on themselves
  const who = await requireScope(req, res, op === 'ooo' ? null : 'projects');
  if (!who) return;
  ...
  try {
    ...
    res.status(400).json({ error: 'Bad op', ops: ['save', 'retire', 'resetCode', 'invite', 'ooo', 'tag', 'untag'] });
  } catch (err) {
