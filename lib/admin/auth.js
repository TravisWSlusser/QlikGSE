...
export async function identify(req) {
  ...
  if (mm) {
    try {
      ...
      try {
        rows = await sql`
          SELECT id, name, trigram, code_hash, code_salt, is_manager FROM team_members
        ...
        rows = await sql`
          SELECT id, name, trigram, code_hash, code_salt, false AS is_manager FROM team_members
          WHERE active = true AND trigram = ${mm[1].toUpperCase()} AND code_hash <> '' LIMIT 1`;
      }
