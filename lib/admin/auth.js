...
export const SCOPES = ['calendar', 'banners', 'content', 'analytics', 'system'];
...
export async function requireScope(req, res, scope) {
  ...
  if (scope && !who.scopes.includes(scope)) {
    res.status(403).json({ error: `This key does not include the '${scope}' scope`, scopes: who.scopes });
    return null;
  }
  return who;
}
