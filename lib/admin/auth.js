...
/*
  What a NON-manager staff member may be granted on their Staff row (v12).

  Deliberately narrower than SCOPES. 'system' closes the REC Room and mints
  keys — the site-fatal class that stays with manager/master/ULTRA, and the
  reason granting editorial access must not go through the Manager checkbox.
  'projects' is out for a different reason: reads there are already open to
  every key holder and a tagged member can already move their own projects,
  so granting it adds only create/retire/tagging, which is a manager call.
*/
export const GRANTABLE = ['calendar', 'banners', 'content', 'analytics'];
...
export async function identify(req) {
  ...
  // 3. Member access codes — `TRI:code`. Checked last. A plain member
  //    grants no scopes; a MANAGER (is_manager, Travis's roster of seven)
  //    holds every scope — full control of the site short of master-only
  //    operations, which stay with ADMIN_KEY/ULTRA. Wrong trigram,
  //    unclaimed member, wrong code, missing table: all of them read as
  //    the same 401 below, never as an oracle.
  const mm = key.match(/^([A-Za-z]{3}):(.{1,64})$/);
  if (mm) {
    try {
      // Walk-back select, one tier per schema bump, never nested deeper
      // (v12 → v3 → v2). The v12 tier must NOT be the only thing carrying
      // is_manager: on a deployed-but-not-yet-Setup database the missing
      // `scopes` column would throw and drop straight to the pre-v3 tier,
      // silently demoting every manager until someone ran Setup.
      const TRI = mm[1].toUpperCase();
      let rows;
      try {
        rows = await sql`
          SELECT id, name, trigram, code_hash, code_salt, is_manager, is_leader, scopes FROM team_members
          WHERE active = true AND trigram = ${TRI} AND code_hash <> '' LIMIT 1`;
      } catch {
        try {
          // v3..v11: manager tier exists, per-member scopes do not
          rows = await sql`
            SELECT id, name, trigram, code_hash, code_salt, is_manager, is_leader,
                   '{}'::text[] AS scopes FROM team_members
            WHERE active = true AND trigram = ${TRI} AND code_hash <> '' LIMIT 1`;
        } catch {
          // pre-v3: member sign-in must keep working regardless
          rows = await sql`
            SELECT id, name, trigram, code_hash, code_salt, false AS is_manager,
                   false AS is_leader, '{}'::text[] AS scopes FROM team_members
            WHERE active = true AND trigram = ${TRI} AND code_hash <> '' LIMIT 1`;
        }
      }
      if (rows.length && verifyCode(mm[2], rows[0].code_salt, rows[0].code_hash)) {
        const manager = !!rows[0].is_manager;
        // A manager still holds everything. Everyone else holds exactly what
        // was granted on their Staff row, re-filtered through GRANTABLE on
        // the way out so a stale row can never carry 'system' or 'projects'.
        const granted = (rows[0].scopes || []).filter(s => GRANTABLE.includes(s));
        return {
          ok: true, label: rows[0].name, scopes: manager ? SCOPES.slice() : granted,
          master: false, manager,
          people_leader: !!rows[0].is_leader, // gates the Leadership Brief
