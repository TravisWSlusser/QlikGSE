...
export async function identify(req) {
  ...
  if (mm) {
    try {
      ...
      try {
        rows = await sql`
          SELECT id, name, trigram, code_hash, code_salt, is_manager, is_leader FROM team_members
          WHERE active = true AND trigram = ${mm[1].toUpperCase()} AND code_hash <> '' LIMIT 1`;
      } catch {
        // pre-v3 database (Setup not yet re-run): the column is missing,
        // but member sign-in must keep working
        rows = await sql`
          SELECT id, name, trigram, code_hash, code_salt, false AS is_manager, false AS is_leader FROM team_members
          WHERE active = true AND trigram = ${mm[1].toUpperCase()} AND code_hash <> '' LIMIT 1`;
      }
      if (rows.length && verifyCode(mm[2], rows[0].code_salt, rows[0].code_hash)) {
        const manager = !!rows[0].is_manager;
        return {
          ok: true, label: rows[0].name, scopes: manager ? SCOPES.slice() : [],
