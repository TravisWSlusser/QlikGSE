...
/* Field caps — the rotator layouts were designed around short copy, and a
   runaway title grows the stage for every slide (it only ever gets taller).
   Enforced by REJECTING, not silent truncation: an editor should see "too
   long", never discover missing words on the live page. Measured on plain
   text (tags stripped) so accent markup doesn't eat the budget. */
const LIMITS = { kicker: 40, title: 60, body: 300, date_text: 40, ctaLabel: 40 };
const plain = s => String(s).replace(/<[^>]*>/g, '');

function cleanCtas(raw) {
  if (!Array.isArray(raw)) return [];
  return raw.slice(0, 4).map(c => ({
    label: (c && c.label ? String(c.label) : '').slice(0, LIMITS.ctaLabel).trim(),
...
export default async function handler(req, res) {
  ...
  const kicker = sanitizeHtml((b.kicker || '').toString());
  const title = sanitizeHtml((b.title || '').toString());
  const body = sanitizeHtml((b.body || '').toString());
  const date_text = (b.date_text || '').toString();
  ...
  if (!plain(title).trim()) return res.status(400).json({ error: 'Title is required' });
  for (const [field, value, cap] of [
    ['Kicker', plain(kicker), LIMITS.kicker],
    ['Title', plain(title), LIMITS.title],
    ['Body', plain(body), LIMITS.body],
    ['Date text', date_text, LIMITS.date_text],
  ]) {
    if (value.length > cap) {
  ...
  try {
    ...
    await logChange(who, 'banners', `${id ? 'Updated' : 'Created'} ${board} post “${plain(title).slice(0, 50)}”`);
