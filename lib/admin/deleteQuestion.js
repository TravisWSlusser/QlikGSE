...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Stamping retired_at starts the deletion window (lib/admin/retireWindow.js);
    // listQuestions sweeps anything past it. Pre-Setup databases have no such
    // column, so fall back to the plain retire rather than refuse the action.
    let rows;
    try {
      if (table === 'questions') {
        rows = await sql`UPDATE questions SET active = false, retired_at = now() WHERE id = ${id} RETURNING id`;
      } else if (table === 'methodology_questions') {
        rows = await sql`UPDATE methodology_questions SET active = false, retired_at = now() WHERE id = ${id} RETURNING id`;
      } else {
        rows = await sql`UPDATE glossary_terms SET active = false, retired_at = now() WHERE id = ${id} RETURNING id`;
      }
    } catch (err) {
    ...
    await logChange(who, 'questions', `Retired #${id} from ${table}`);
