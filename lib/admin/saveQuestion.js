...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Keep the retirement clock in step with `active`, for both the insert and
    // the update path. Restoring clears the stamp; retiring via an edit starts
    // it; editing a row that is ALREADY retired leaves its existing stamp alone
    // (COALESCE), so a typo fix does not buy the row another 48 hours.
    // Pre-Setup databases have no retired_at — swallowed, same as elsewhere.
    try {
      if (table === 'questions') {
        await sql`UPDATE questions SET retired_at =
          CASE WHEN active THEN NULL ELSE COALESCE(retired_at, now()) END WHERE id = ${row.id}`;
      } else if (table === 'methodology_questions') {
        await sql`UPDATE methodology_questions SET retired_at =
          CASE WHEN active THEN NULL ELSE COALESCE(retired_at, now()) END WHERE id = ${row.id}`;
      } else {
        await sql`UPDATE glossary_terms SET retired_at =
          CASE WHEN active THEN NULL ELSE COALESCE(retired_at, now()) END WHERE id = ${row.id}`;
      }
    } catch (err) {
