  ...
  try {
    if (op === 'list') {
      ...
      if (ids.length) {
        try {
          yarn = await sql`
            SELECT id, from_id, to_id, color, from_ax, from_ay, to_ax, to_ay FROM sticky_yarn
            WHERE active = true AND board_no = ${boardNo}
    ...
    if (op === 'save') {
      ...
      // a member session signs with the credential's name, whatever the
      // client sent — signed in means the system knows who you are
      const poster_name = who.member ? who.member.name
        : (b.poster_name || '').toString().trim();
    ...
    if (op === 'react') {
      ...
      const name = who.member ? who.member.name : (b.name || '').toString().trim();
    ...
    if (op === 'tie') {
      ...
      // anchors: where on each item the string is pinned, as 0..1 fractions
      // of its box; absent means the client clips to the item's edge
      const frac = v => { const n = Number(v); return Number.isFinite(n) ? Math.max(0, Math.min(1, n)) : null; };
      const from_ax = frac(b.from_ax), from_ay = frac(b.from_ay);
      const to_ax = frac(b.to_ax), to_ay = frac(b.to_ay);
      const rows = await sql`
        INSERT INTO sticky_yarn (from_id, to_id, color, board_no, from_ax, from_ay, to_ax, to_ay)
        VALUES (${from_id}, ${to_id}, ${color}, ${boardNo}, ${from_ax}, ${from_ay}, ${to_ax}, ${to_ay}) RETURNING id`;
