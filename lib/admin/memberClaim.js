...
export default async function handler(req, res) {
  ...
  if (!invite || invite.length > 20) return res.status(400).json({ error: 'The activation code from your manager is required' });
  ...
  try {
    ...
    try {
      rows = await sql`
        SELECT id, name, invite_hash, invite_salt, invited_at, invited_by FROM team_members
        WHERE active = true AND trigram = ${trigram} LIMIT 1`;
    } catch {
      // pre-v5 database: invited_by not there yet — claiming must still work
      rows = await sql`
        SELECT id, name, invite_hash, invite_salt, invited_at, '' AS invited_by FROM team_members
        WHERE active = true AND trigram = ${trigram} LIMIT 1`;
    }
    if (!rows.length) {
    ...
    const m = rows[0];
    if (!m.invite_hash) {
      return res.status(403).json({ error: 'Access is by activation key — ask a manager for yours' });
    }
    const age = m.invited_at ? (Date.now() - new Date(m.invited_at).getTime()) : Infinity;
    if (age > INVITE_DAYS * 24 * 3600 * 1000) {
      return res.status(410).json({ error: 'That activation code has expired — ask a manager for a fresh one' });
    }
    if (!verifyCode(invite, m.invite_salt, m.invite_hash)) {
      return res.status(403).json({ error: 'That activation code is not right' });
    }
    const salt = makeSalt();
    ...
    await logChange({ label: `${m.name} (gate)` }, 'projects',
      `${m.name} redeemed their invite code sent by ${m.invited_by || 'a manager'}`);
