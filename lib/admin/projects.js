...
export default async function handler(req, res) {
  ...
  try {
    if (op === 'list') {
      ...
      let members = [], tags = [], recs = [], staffReacts = [];
      // the members select walks back through schema versions until one
      // fits — a deploy always lands before Setup runs, and the Board
      // must serve either way
      const MEMBER_SELECTS = [
        () => sql`SELECT id, name, trigram, team_id, title, email, active,
          is_leader, is_manager, manager_id, avatar_url, ooo_note, status_text, status_at,
          scopes, (code_hash <> '') AS claimed FROM team_members ORDER BY name`,   // v12
        () => sql`SELECT id, name, trigram, team_id, title, email, active,
          is_leader, is_manager, manager_id, avatar_url, ooo_note, status_text, status_at,
          (code_hash <> '') AS claimed FROM team_members ORDER BY name`,           // v8
        () => sql`SELECT id, name, trigram, team_id, title, email, active,
          is_leader, is_manager, manager_id, avatar_url, ooo_note,
          (code_hash <> '') AS claimed FROM team_members ORDER BY name`,           // v6
        () => sql`SELECT id, name, trigram, team_id, title, email, active,
          is_leader, is_manager, manager_id, (code_hash <> '') AS claimed FROM team_members ORDER BY name`, // v3
        () => sql`SELECT id, name, trigram, team_id, title, email, active,
          is_leader, manager_id, (code_hash <> '') AS claimed FROM team_members ORDER BY name`,             // v2
      ];
      try {
        for (const q of MEMBER_SELECTS) {
          try { members = await q(); break; } catch { /* older schema — walk back */ }
        }
        tags = await sql`SELECT project_id, member_id FROM project_members WHERE active = true`;
      } catch { /* re-run Setup for the registry */ }
      try {
        staffReacts = await sql`SELECT member_id, emoji, name FROM staff_status_reactions ORDER BY id`;
      } catch { /* pre-v8 — no reactions yet */ }
      ...
      return res.status(200).json({ projects, teams, statuses, milestones, members, tags, recs, staffReacts, today });
    }
