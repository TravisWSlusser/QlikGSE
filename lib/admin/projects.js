...
export default async function handler(req, res) {
  ...
  const who = await requireScope(req, res, READ_OPS.includes(op) ? null : 'projects');
  if (!who) return;
  ...
  try {
    if (op === 'list') {
      ...
      const teams = await sql`SELECT id, name, sort, active FROM project_teams ORDER BY sort, id`;
      ...
      let members = [], tags = [];
      try {
        members = await sql`SELECT id, name, trigram, team_id, title, active FROM team_members ORDER BY name`;
        tags = await sql`SELECT project_id, member_id FROM project_members WHERE active = true`;
      } catch { /* re-run Setup for the registry */ }
      return res.status(200).json({ projects, teams, statuses, milestones, members, tags, today });
    }
    ...
    if (op === 'deleteMilestone') {
      const id = Number(b.id);
      const rows = await sql`UPDATE project_milestones SET active = false WHERE id = ${id} AND active = true RETURNING title`;
      if (!rows.length) return res.status(404).json({ error: 'No such milestone', id });
      return res.status(200).json({ ok: true, id });
    }
