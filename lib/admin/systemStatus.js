...
export default async function handler(req, res) {
  ...
  const probes = {
    ...
    blob: async () => {
      if (!process.env.BLOB_READ_WRITE_TOKEN) return { name: 'Vercel Blob', group: 'Core', status: 'off', detail: 'No token configured' };
      const t0 = Date.now();
      await T(list({ limit: 1 }));
    ...
    resend: async () => {
      ...
      const t0 = Date.now();
      const r = await T(fetch('https://api.resend.com/domains', {
        headers: { Authorization: `Bearer ${process.env.RESEND_API_KEY}` },
      }));
      if (r.status === 401) return { name: 'Resend email', group: 'Core', status: 'down', detail: 'API key rejected' };
      if (!r.ok) return { name: 'Resend email', group: 'Core', status: 'warn', detail: `Responded ${r.status}` };
