...
export default async function handler(req, res) {
  ...
  const probes = {
    ...
    blob: async () => {
      ...
      // list() is a PAID Advanced operation — 2,000/month on the free tier.
      // The in-memory 60s cache dies with every cold lambda, so per-poll
      // probing burned 75% of the month's quota in about a day of an open
      // dashboard (2 Sep 2026). Same doctrine as the news probe: the real
      // ping runs at most every 6h (30m after a failure), throttled through
      // the DB so every lambda shares one clock.
      try {
        const rows = await T(sql`SELECT payload, EXTRACT(EPOCH FROM (now() - fetched_at))/60 AS age
          FROM api_cache WHERE key = 'blob_probe' LIMIT 1`);
        if (rows.length) {
          const age = Number(rows[0].age);
          const p = rows[0].payload || {};
          const ttl = p.status === 'ok' ? 360 : 30;
          if (age < ttl) return { name: 'Vercel Blob', group: 'Core', status: p.status,
            detail: `${p.detail} — verified ${ago(age)}` };
        }
      } catch { /* cache unreadable: fall through to a live probe */ }
      let probe;
      const t0 = Date.now();
      try {
        await T(list({ limit: 1, token }));
        probe = { status: 'ok', detail: 'Image store reachable' };
      } catch (e) {
        probe = { status: 'down', detail: String(e && e.message || e).slice(0, 60) };
      }
      try {
        await sql`INSERT INTO api_cache (key, payload, fetched_at)
          VALUES ('blob_probe', ${JSON.stringify(probe)}::jsonb, now())
          ON CONFLICT (key) DO UPDATE SET payload = EXCLUDED.payload, fetched_at = now()`;
      } catch { /* no cache table: next poll probes again */ }
      return { name: 'Vercel Blob', group: 'Core', ...probe, ms: Date.now() - t0 };
    },
    ...
    news: async () => {
      ...
      try {
        const rows = await T(sql`SELECT EXTRACT(EPOCH FROM (now() - fetched_at))/60 AS age FROM api_cache WHERE key = 'news' LIMIT 1`);
