...
export default async function handler(req, res) {
  ...
  try {
    if (op === 'saveTeam') {
      ...
      // the team's leader, for the catalog hierarchy — optional, must be a
      // live registry member when set
      let leader_id = Number(b.leader_id) || null;
      if (leader_id) {
        const lm = await sql`SELECT id FROM team_members WHERE id = ${leader_id} AND active = true LIMIT 1`;
        if (!lm.length) return res.status(400).json({ error: 'That leader is not an active team member' });
      }
      ...
      let tid = id;
      if (id) {
        const rows = await sql`UPDATE project_teams SET name = ${name}, sort = ${sort}, leader_id = ${leader_id}
          WHERE id = ${id} RETURNING id`;
        if (!rows.length) return res.status(404).json({ error: 'No such team', id });
      } else {
        const rows = await sql`INSERT INTO project_teams (name, sort, leader_id, created_by)
          VALUES (${name}, ${sort}, ${leader_id}, ${who.label}) RETURNING id`;
        tid = rows[0].id;
      }
