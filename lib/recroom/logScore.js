...
export default async function handler(req, res) {
  ...
  // Session keys resolve through lib/secrets.js: CAPCOM's Keys & Services
  // row first, the Vercel env var as fallback.
  const KEYS = [await getSecret('MT_SESSION_REF'), await getSecret('MT_SESSION_REF_MOBILE')]
  ...
  try {
    ...
    // Cumulative playtime for CAPCOM's dashboard. Separate, NON-FATAL write:
    // the column arrives via CAPCOM's migrate, and a score must never fail
    // because the stats column doesn't exist yet.
    try {
      await sql`UPDATE players SET play_seconds = COALESCE(play_seconds, 0) + ${durationSec}
        WHERE trigram = ${trigram}`;
    } catch (e) { /* pre-migrate — ignore */ }
