...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Identity for the hover cards — joined per ROW shown, so the full
    // roster (it is employee data) is never served. Sub-wrapped: a
    // missing rec_roster table must never take the boards down.
    try {
      const tris = [...new Set([...topByTerritory, ...gameMasters].map(p => p.trigram))];
      if (tris.length) {
        ...
        topByTerritory.forEach(tag); gameMasters.forEach(tag);
      }
