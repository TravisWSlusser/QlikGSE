...
export default async function handler(req, res) {
  ...
  try {
    ...
    const ok = choice === correctKey;
    // Per-question stats for the Control Room's miss-rate readout.
    // Non-fatal, like the score_events insert: an answer must never fail
    // because bookkeeping did (or because migrate hasn't added the columns).
    try {
      await sql`UPDATE questions SET attempted = attempted + 1,
        correct = correct + ${ok ? 1 : 0} WHERE id = ${id}`;
    } catch (e) { /* ignore */ }
    return res.status(200).json({ correct: ok, correct_option: correctKey });
  } catch (e) {
