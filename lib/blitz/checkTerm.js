...
export default async function handler(req, res) {
  ...
  try {
    ...
    const ok = choice === String(term).trim().toLowerCase();
    // Per-question stats for the Control Room — non-fatal (see checkAnswer).
    try {
      await sql`UPDATE glossary_terms SET attempted = attempted + 1,
