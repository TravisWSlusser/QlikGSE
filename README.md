...
| **CAPCOM** | `CAPCOM/` | *Content, Analytics & Planning — Command Operations Module.* The team's command center: calendar and banner editors, question banks, the Project Board (promises with dates, append-only diary, Gantt and insights), the Staff org chart (avatars, out-of-office, status posts with reactions), a Leadership Brief with optional Claude-written executive summaries, the MT Roster, per-page guided walkthroughs, Help & FAQ with bug reports, and the Community Board. Installable as a PWA, responsive down to a phone. |
...
CAPCOM is invite-only — leaders issue one-time activation keys, members set
their own passwords (scrypt-hashed), and scoped keys cover SMEs and services —
all of it failing closed. Everything user-visible degrades
