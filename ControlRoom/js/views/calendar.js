...
function editEvent(e, cats, rerender) {
  ...
  const f = {
    ...
    title: textInput({ value: e.title, placeholder: 'Short — the chip has limited width' }),
    detail: textArea({ value: e.detail, rows: 3 }),
    full_copy: textArea({ value: e.full_copy, rows: 5 }),
    link: textInput({ value: e.link, placeholder: 'https://… (renders as the modal’s link)' }),
  ...
  modal(isNew ? 'New event' : 'Edit event',
    h('div', { class: 'form' },
      ...
      field('Title', f.title),
      field('Detail', f.detail, 'One or two sentences — shows in the chip expansion and the calendar modal.'),
      field('Long-form copy (optional)', f.full_copy, 'Stored for the future long-form view. Not rendered anywhere yet.'),
      field('Link (optional)', f.link),
